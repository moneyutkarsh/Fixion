...
themeStyle.textContent = `
    ...
    @keyframes fx-pulse-red { 0% { background: rgba(220, 38, 38, 0.4); } 50% { background: rgba(255, 0, 0, 0.7); } 100% { background: rgba(220, 38, 38, 0.4); } }
`;
...
function updatePanel(data) {
    ...
    if (!content) return;
    const rel = (data.scoring?.reliability_score || 0) * 100;
    const hal = (data.scoring?.hallucination || 0) * 100;
    // Color logic: High hallucination is bad (Red), Low is good (Green)
    const meterColor = hal > 50 ? "#f43f5e" : (hal > 20 ? "#f59e0b" : "#10b981");

    content.innerHTML = `
        <div style="animation: fx-fade-slide 0.5s ease-out;">
            <div style="margin-bottom: 24px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                    <div style="color: #94a3b8; font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px;">Hallucination Index</div>
                    ${hal > 20 ? `<div style="background: rgba(244, 63, 94, 0.15); color: #f43f5e; padding: 4px 12px; border-radius: 8px; font-size: 11px; font-weight: 800; border: 1px solid rgba(244, 63, 94, 0.2);">High Risk</div>` : '<div style="color: #10b981; font-size: 10px; font-weight: 800;">✓ Verified Safe</div>'}
                </div>
                <div style="font-size: 56px; font-weight: 900; color: #fff; line-height: 1;"><span id="fx-live-hall-pct">${hal.toFixed(2)}</span>%</div>
                <div style="height: 6px; background: rgba(255,255,255,0.05); border-radius: 3px; margin-top: 12px; overflow: hidden;">
                    <div style="height: 100%; width: ${hal}%; background: ${meterColor}; transition: width 1s;"></div>
                </div>
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 24px;">
                <div style="padding: 16px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; text-align: center;">
                    <div style="color: #94a3b8; font-size: 9px; font-weight: 800; text-transform: uppercase; margin-bottom: 4px;">Verified Info</div>
                    <div style="font-size: 24px; font-weight: 800; color: #10b981;">${rel.toFixed(1)}%</div>
                </div>
                <div style="padding: 16px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; text-align: center;">
    ...
    document.getElementById("fx-highlight-btn").onclick = () => {
        ...
        console.log("Fixion: Running Fuzzy Agnostic Highlighting...");
        let currentHTML = originalHTML;

        data.nli_results.forEach(res => {
            if (res.status !== 'contradiction') return; // FOCUS ON RED HIGHLIGHTS
            
            const targetText = res.claim.trim();
            if (targetText.length < 5) return;

            // FUZZY REGEX: Allows for HTML tags and whitespace between words
            const words = targetText.split(/\s+/).filter(w => w.length > 0);
            const regexStr = words
                .map(w => w.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
                .join('(?:<[^>]+>|\\s)+');
            
            try {
                const regex = new RegExp(`(${regexStr})`, 'gi');
                currentHTML = currentHTML.replace(regex, `<span style="background: #ff0000 !important; color: #ffffff !important; font-weight: 900 !important; padding: 1px 3px; border-radius: 4px; animation: fx-pulse-red 2s infinite; box-shadow: 0 0 12px rgba(255,0,0,0.6); border-bottom: 3px solid #7f1d1d !important;">$1</span>`);
                console.log("Fixion: Fuzzy matched and highlighted:", targetText);
            } catch (e) {
                console.warn("Fixion: Highlight failure for:", targetText, e);
            }
        });
        
        latestElement.innerHTML = currentHTML;
    ...
    const listener = (msg) => {
        if (msg.type === "STREAM_CHUNK") {
            try {
                ...
                if (event.step === "nli_done") { 
                    updateStage("nli", "done"); 
                    updateStage("scoring", "active"); 
                }
                if (event.step === "partial_scoring") {
                    const hallPct = event.data.hallucination * 100;
                    const el = document.getElementById("fx-live-hall-pct");
                    if (el) el.innerText = hallPct.toFixed(2);
                }
